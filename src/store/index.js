...
const store = new Vuex.Store({
    ...
    mutations: { // 与 state 同步执行；可以改变 state 对应的值的方法
        ...
        changecurrentSession(state, currentSession) {
            ...
            console.log('currentAdmin=>', JSON.stringify(state.currentAdmin));
            // 已读消息 小红点消失
            if (state.currentAdmin == null) {
                state.currentAdmin = JSON.parse(window.sessionStorage.getItem('user'));
            }
    ...
    actions: {
        ...
        connect(context) {
            ...
            context.state.stomp.connect({'Auth-Token': token}, success => { // eslint-disable-line no-unused-vars
                // 把 token 放进去
                context.state.stomp.subscribe('/user/queue/chat', msg => { // 订阅消息频道
                    ...
                    if (!context.state.currentSession || receiveMsg.from !== context.state.currentSession.username
                        ...
                        console.log('receiveMsg=>', JSON.stringify(receiveMsg));
                        Notification.info({
        ...
        initData(context) {
            ...
            // 1-4 调用接口获取数据
            getRequest('/api/chat/admin').then(resp => {
                if (resp) {
