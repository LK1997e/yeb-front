    ...
    <div class="positionManaMain">
      <el-collapse v-model="activeName" accordion @change="change">
        <el-collapse-item :name="r.id" v-for="(r,index) in roles" :key="index">
          <template slot="title">
            <span style="margin-left: 10px"> {{r.nameZh}}</span>
          </template>
...
<script>
import {reloadMenu} from "@/utils/menus";
  ...
  methods: {
    ...
    doDeleteRole(role){
      this.$confirm('此操作将永久删除[' + role.nameZh + ']角色, 是否继续?', '提示', {
        ...
        this.deleteRequest('/api/system/basic/permission/role/' + role.id).then(resp => {
          if (resp) {
            this.initRoles()
          }
        })
    ...
    doAddRole(){
      if (this.role.name && this.role.nameZh) {
        this.postRequest('/api/system/basic/permission/role',this.role).then(resp=>{
          if (resp) {
            this.initRoles()
            this.role.name = ''
            this.role.nameZh = ''
    ...
    doUpdate(rid, index) {
      ...
      this.putRequest(url).then(resp => {
        if (resp) {
          let roles = JSON.parse(window.sessionStorage.getItem('user')).roles;
          roles.forEach(role=>{
            if(role.id == rid){
              reloadMenu();
            }
    ...
    initSelectedMenus(rid) { // :name="r.id"  change(rid)
      this.getRequest('/api/system/basic/permission/mid/' + rid).then(resp => {
        if (resp) {
    ...
    // 获取所有菜单
    initAllMenus() {
      this.getRequest('/api/system/basic/permission/menus').then(resp => {
        if (resp) {
          this.allMenus = resp
        }
      })
    ...
    // 获取所有角色
    initRoles() {
      this.getRequest('/api/system/basic/permission/').then(resp => {
        if (resp) {
          this.roles = resp
        }
      })
