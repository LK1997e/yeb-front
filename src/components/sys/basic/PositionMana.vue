  ...
  methods: {
    ...
    deleteMany() {
      this.$confirm('此操作将永久删除[' + this.multipleSelection.length + ']条职位, 是否继续?', '提示', {
        ...
        this.deleteRequest('/api/system/basic/pos/' + ids).then(resp => {
          if (resp) {
            this.initPositions()
          }
        })
    ...
    // 编辑职位
    doUpdate() {
      this.putRequest('/api/system/basic/pos/', this.updatePos).then(resp => {
        if (resp) {
    ...
    addPosition() {
      if (this.pos.name) {
        this.postRequest('/api/system/basic/pos/', this.pos).then(resp => {
          if (resp) {
            this.initPositions()
            this.pos.name = ''
          }
    ...
    handleDelete(index, data) {
      this.$confirm('此操作将永久删除[' + data.name + ']职位, 是否继续?', '提示', {
        ...
        this.deleteRequest(' /api/system/basic/pos/' + data.id).then(resp => {
          if (resp) {
            this.initPositions()
          }
        })
    ...
    // 获取后端接口所有职位数据
    initPositions() {
      this.getRequest('/api/system/basic/pos/').then(resp => {
        if (resp) {
          this.positions = resp
        }
      })
