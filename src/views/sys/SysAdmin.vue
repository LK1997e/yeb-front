  ...
  methods: {
    ...
    hidePop(admin) {
      ...
      if (flag) {
        ...
        let url = '/api/system/admin/role?adminId=' + admin.id;
    ...
    // 15、更新操作员
    enabledChange(admin) {
      this.putRequest('/api/system/admin/', admin).then(resp => {
        if (resp) {
          this.initAdmins()
        }
      })
    ...
    deleteAdmin(admin) {
      this.$confirm('此操作将永久删除该[' + admin.name + '], 是否继续?', '提示', {
        ...
        this.deleteRequest('/api/system/admin/' + admin.id).then(resp => {
          if (resp) {
            this.initAdmins()
          }
        })
    ...
    // 4、获取所有操作员；11、加参数关键字
    initAdmins() {
      this.getRequest('/api/system/admin/?keywords=' + this.keywords).then(resp => {
        if (resp) {
          this.admins = resp
        }
      })
...
.admin-container {
  margin-top: 10px;
  display: flex;
  justify-content: left; /* 在弹性盒对象的 <div> 元素中的各项周围留有空白：*/
  flex-wrap: wrap; /* 自动换行 */
}
