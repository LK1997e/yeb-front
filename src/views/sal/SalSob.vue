<template>
  <div>
    ...
    <el-dialog
        ...
        width="50%">
      <!-- 3-8 调整样式 -->
      <div style="display: flex;justify-content: space-between;align-items: center; padding: 0px 50px">
        <!-- 3-1 添加步骤条 -->
        <!-- 3-5  :active="activeItemIndex" -->
        <el-steps direction="vertical" :active="activeItemIndex">
          <!-- 3-3 循环遍历数据 -->
          <el-step :title="itemName" v-for="(itemName,index) in salaryItemName" :key="index">
            <span slot="description"
                  v-if="salary[salaryItemEnName[index]]!=null && salary[salaryItemEnName[index]]+''!=0"
                  style="color: darkgray;">{{ salary[salaryItemEnName[index]] }}</span>
          </el-step>
          <el-step title="完成" :key="salaryItemName.length"></el-step>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button @click="cancelAdd" v-if="activeItemIndex === salaryItemName.length">取消</el-button>
        <el-button @click="preStep" v-if="activeItemIndex > 0">上一步</el-button>
        <el-button @click="nextStep" v-if="activeItemIndex < salaryItemName.length">下一步</el-button>
        <el-button type="primary" @click="finishAdd" v-if="activeItemIndex == salaryItemName.length">完成</el-button>
      </span>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    deleteSalary(data) {
      this.$confirm('此操作将永久删除该[' + data.name + ']工资账套, 是否继续?', '提示', {
        ...
        this.deleteRequest('/api/salary/sob/' + data.id).then(resp => {
          if (resp) {
            this.initSalaries()
          }
        })
    ...
    nextStep() { // 3-12 下一步 完成
      if (this.activeItemIndex >= 0) {
        let currentSalaryItemName = this.salaryItemName[this.activeItemIndex];
        let value = this.salary[this.salaryItemEnName[this.activeItemIndex]];
        if (value == null || (value + '').trim() == '') {
          Message.warning(currentSalaryItemName + '不能为空');
          return;
        }
      }
    ...
    finishAdd() {
      ...
      if (this.salary.id) { // 6-6 有 id 调用编辑接口，没有 id 执行添加
        this.putRequest('/api/salary/sob/', this.salary).then(resp => {
          if (resp) {
            this.initSalaries()
        ...
        this.postRequest('/api/salary/sob/', this.salary).then(resp => {
          if (resp) {
            this.initSalaries()
            this.dialogVisible = false
          }
    ...
    // 1-3 初始化数据
    initSalaries() {
      this.getRequest('/api/salary/sob/').then(resp => {
        if (resp) {
          this.salaries = resp
        }
      })
