  ...
  methods: {
    ...
    hidePop(data) { // 隐藏时触发
      ...
      if (this.currentSalary && this.currentSalary!==data.salary.id) {
        this.putRequest('/api/salary/sobcfg/?eid=' + data.id + '&sid=' + this.currentSalary).then(resp => {
          if (resp) {
            this.initEmps()
          }
        });
    ...
    // 2-3 获取所有工资账套
    initSalaries() {
      this.getRequest('/api/salary/sobcfg/salaries').then(resp => {
        if (resp) {
          this.salaries = resp
        }
      })
    ...
    // 获取所有数据
    initEmps() {
      this.getRequest('/api/salary/sobcfg/?currentPage=' + this.currentPage + '&size=' + this.size).then(resp => {
        if (resp) {
          this.emps = resp.data
