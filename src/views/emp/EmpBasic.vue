        ...
        <div v-show="showAdvanceSearchVisible"
          ...
          <el-row style="margin-top: 10px;">
            ...
            <el-col :span="5" :offset="4">
              <el-button size="mini" @click="cancelAdvancedInfo">取消</el-button>
...
<!--            <el-button style="padding: 3px;" size="mini" type="primary" plain>查看高级资料</el-button>-->
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="doAddEmp" v-prevent-re-click>确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 26-2 下载请求
    exportData() {
      this.downloadRequest('/api/employee/basic/export')
    },
    // 25-5 编辑员工按钮 点击事件
    showEmpView(data) {
      this.title = '编辑员工信息'
    ...
    deleteEmp(data) {
      this.$confirm('此操作将永久删除该员工' + data.name + ', 是否继续?', '提示', {
        ...
        this.deleteRequest('/api/employee/basic/' + data.id).then(resp => {
          if (resp) {
            this.initEmps()
          }
        })
    ...
    doAddEmp() {
      if (this.emp.id) {
        ...
        this.$refs['empRef'].validate(valid => {
          if (valid) {
            this.putRequest('/api/employee/basic/', this.emp).then(resp => {
              if (resp) {
        ...
        this.$refs['empRef'].validate(valid => {
          if (valid) {
            this.postRequest('/api/employee/basic/', this.emp).then(resp => {
              if (resp) {
                this.dialogVisible = false
                this.initEmps()
              }
    ...
    // 23-13 添加员工 获取最大号
    getMaxWorkID() {
      this.getRequest('/api/employee/basic/maxWorkID').then(resp => {
        if (resp) {
    ...
    initPositions() {
      this.getRequest('/api/employee/basic/positions').then(resp => {
        if (resp) {
          this.positions = resp
        }
      })
    ...
    initData() {
      ...
      if (!window.sessionStorage.getItem("nations")) {
        this.getRequest('/api/employee/basic/nations').then(resp => {
          this.nations = resp
      ...
      if (!window.sessionStorage.getItem('joblevels')) {
        this.getRequest('/api/employee/basic/joblevels').then(resp => {
          if (resp) {
            this.joblevels = resp
      ...
      if (!window.sessionStorage.getItem('politicsstatus')) {
        this.getRequest('/api/employee/basic/politicsStatus').then(resp => {
          if (resp) {
      ...
      if (!window.sessionStorage.getItem('allDeps')) {
        this.getRequest('/api/employee/basic/deps').then(resp => {
          if (resp) {
            this.allDeps = resp
    ...
    initEmps(type) {
      ...
      // 30-11 定义高级搜索 url
      let url = '/api/employee/basic/?currentPage=' + this.currentPage + '&size=' + this.size
